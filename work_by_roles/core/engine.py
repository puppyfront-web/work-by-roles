...
class SkillSelector:
    ...
    def _get_available_skills(self, role: Role) -> List[Skill]:
        ...
        skills = []
        # New format: role.skills is a list of skill IDs
        for skill_id in role.skills:
...
class Agent:
    ...
    def produce_output(self, name: str, content: str, output_type: str = "code", stage_id: Optional[str] = None):
        ...
        if output_type in ("document", "report") and not getattr(self.engine, 'generate_document_files', True):
            # Only store in context, don't write to disk
            self.context.outputs[name] = f"[preview_only:{name}]"
            print(f"📄 Document preview generated: {name} (not saved to disk)")
            return
...
# AgentOrchestrator is now imported from .agent_orchestrator module
# The old class definition has been removed to avoid conflicts
